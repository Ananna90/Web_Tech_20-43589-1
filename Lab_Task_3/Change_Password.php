...
if(empty($p) || empty($np) || empty($cp)){
        $Err = "Please fill up the informations.";
    }
    else{
        if(!preg_match( "/^(?=.*[A-Za-z])(?=.*\d)(?=.*[@$!%*#?&])[A-Za-z\d@$!%*#?&]{8,}$/", $p)){
        $pErr =" Minimum eight characters,one number and one special character!";
         }
        if(!strcmp($p, $np)){
          $npErr = "Current Password and New Password should not be same.";
         }
        if(strcmp($np, $cp))
         {
...
?>
    ...
    <fieldset>
        ...
        <label for="npass"style="color: black"> <b>New Password :</b> </label>
        ...
        <span class="underline">________________________________________________________________________________________</span><br><br> 
